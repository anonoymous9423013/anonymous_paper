...
def fitnessForAllScenarios(platform='PID', csv_file=None, normalize=True, repeat=10):
    ...
    if platform == 'PID':
        ...
        if normalize:
            # arr = normalizeStd(arr)
            arr = normalizeMinMax(arr)
    elif platform == 'BeamNG_Competition':
        df = pd.read_csv(csv_file)
        result = []
        ...
        for scenario in unique:
            result.append([scenario])
            fitnesses = df[(df.iloc[:, :-1].values == np.array(scenario)).all(axis=1)][output_cols].values
            fitnesses = fitnesses[:repeat]
            for f_row in fitnesses:
                result[-1].append(f_row.tolist())
        t1 = time.time()
...
if __name__ == '__main__':
    ...
    # platform = 'BeamNG'
    platform = 'BeamNG_Competition'
    csv_file = f'ds_{platform.lower()}.csv'
    result_detailed, f_arr_detailed = fitnessForAllScenarios(platform=platform, csv_file=csv_file, normalize=False)  ##  f_arr_detailed.shape = (no_of_scenarios, no_of_repeats, no_of_fitnesses)
    scenarios, f_arr = minAndMeanFitnesses(result_detailed)  ##  f_arr.shape = (no_of_scenarios, 2, no_of_fitnesses)
    n_fitnesses = f_arr.shape[-1]
    df = pd.DataFrame(f_arr_detailed.reshape(-1, f_arr_detailed.shape[-1]), columns=[f'F{i+1}' for i in range(n_fitnesses)])
